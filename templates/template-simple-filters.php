...
<div id="filters">
    ...
    <a class="btn-clear-filters" href="#"> <?php esc_html_e('Clear Filters', 'fast-ajax-filter') ?></a>
</div>
<?php
...
foreach( $filters_order as $index_number ) {
    ...
    if( $index_number == 'categories') {
        if( get_option('fwf_categories') ) {
            echo '<h5>'. esc_html('Categories', 'fast-ajax-filter') . '</h5>';
